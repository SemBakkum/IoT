...
The purpose of this project is this thing that annoyed me about my mother. Most of the times, when she goes to do some work in the garden, she leaves the door open. If we would live in a country where the weather is always this wouldn't be a problem. But since we don't have that kind of luck, you could call it ennoying when the cold gets inside. So I thought: why not build an alarm that lets her know when she left the door open too long and it is starting to get cold inside. The result of the idea made me write this manual for the ones interested in such an alarm.
...
Let's start with the hardware that is necessary to complete this project. 
...
In this tutorial the switch is connected to an Arduino board, but we're connecting it to an NodeMCU, so the first thing we should change is the pin number that our door switch and LED are connected to. In the image below I connected the door switch to pin (orange jumper wires) D1 and the LED to pin D2. We connect them to the D pins, because these sensors give us digital in/output. The black jumper wire is connected to the ground. 
...
I connected the Buzzer to pin D3 and to the ground of the breadboard. The next step is to let it produce some sounds for us, but that will be shown in step 3.

### Let the alarm go off when the door opens (step 3)

The idea of this alarm is that it will go off after a certain time when the door is being open for too long. This means that we don't want it to go off immediately.
To accomplish this we can use the millis() function, instead of using a delay. A delay will delay the rest of the code from running that is declared after it. Millis() doesn't give us this problem, but first we want our alarm (Buzzer or piezo) to make a sound when the door is open. For the sound that we want, I used the code provided in this [link](https://www.arduino.cc/en/tutorial/melody). There is also an good explanation of how the code works. So if we merge our previous code with this code the result will look like the code below.
...
As mentioned in step 3 we want to let the alarm go off after a certain time the door is open. This can be done with millis(). The only thing we have to do is make a simple mathmetical calculation, but first let me tell how millis() works. Once millis() is declared it will start running a time in milliseconds. We can use this time to calculate and trigger certain events in the code without delaying the code. How the code is built up at this stage will not quite work for us, so we've got to refactor it a little bit. The main thing we will be refactoring is the void loop(). The code will look like the one below.
...
We also use a check "isOpen" that can be true or false. So what we do here is say if the door is open that the boolean isOpen is true. Then set isOpen to true and set openMillis, that was declared to be zero, to currentMillis. At this point currentMillis is still running and openMillis just copies the time it was given from that point. So if the door is open and the boolean isOpen is true we can do our mathmetical calculation. That calculation goes as following: if currentMillis - openMillis is bigger or the same as interval (which we have set to be 5000) and isOpen (which is been set to true) are all true you can let the alarm go off. If we want to wait longer for the alarm to go off, we can just set the interval state to a bigger number, lets say 7000. That way the alarm will wait 7 seconds before going off.

Eventually we want the sound to play normal twice. After the second time we want it tot speed up. To accomplish this we add some extra code we have already written and add a few extra things. The code below shows how this can be achieved. 
...
So we declared a playCount=0; this keeps track how many times the sound played or alarm went off. Each time the door is closed playCount is set to 0, but if the door is open and the alarm starts playing we increase playCount by 1. When playCount is 2 we can use the new piece of code to play the alarm faster. In that statement we check if playCount is equal to or bigger than 2. If that is true we play the alarm sound, but we play it twice as fast. As you can see we can do this by dividing temp by 2. 
...
To use this code you should include the ESP8266WiFi.h library. You can use your own hotspot or home WiFi to connect it with the following code.
```javascript
...
In the code above we are doing a GET request to host2 which is www.yourdomain.com/IoT/door.json. This file contains the status of the door, either it is open or closed. Thanks to the JSON library we can decode this to a string in the GET function and store this in the doorStatus. The value of doorStatus is used in the void loop() as an extra check to make sure the alarm can go off, because in the browser we give the user an option to turn the alarm off in case it is nice weather outside and you just want to chill in your garden. If the user turns off the alarm in the browser, that means the doorStatus isn't open, but closed. In PHP we write away what the status is in the door.json file on the server. 

In the POST request we send a value to the server. As you can see in the void loop(), this value can contain 1 or 0. The 1 meaning the door is open and the 0 meaning the door is closed. Our index.php responds to this POST request, but more about that in the following step. We are now done with programming for the NodeMCU and ready to start programming PHP for our browser the user can interact with. 

### Time for some PHP (step 7)
#### Post request in PHP

As mentioned above we send (POST) something to the server and we retrieve (GET) something from the server. The way we do that is by using PHP. For this project we created three files:
...
If the value is equal to 1 we fill the $status variable with the value open. If the value is not equeal to 1 we fill the $status variable with the value closed.
After this we open the door.json file and change the door value to the given value in the $status variable. Then we decode the history.json so that it is writeable for PHP. We fill the satus in the history file with the $status variable and for the created_at we give a new time. When this is done we encode the history.json file to be JSON again. 

#### Get request in PHP

As seen in the code above everything is put in as if statement, so logic would be that when there isn't a POST request there could be a GET request. The other statement there by will contain our GET request. The code looks like following:
...
1. [Door sensor:](http://www.instructables.com/id/Magnetic-Door-Sensor-and-Arduino/?ALLSTEPS) This link helped me to start this project. This manual described how you can turn a led on or off depending if the door is opened or closed. Great link to get you started and it is basically the foundation of my alarm.

2. [Melody:](https://www.arduino.cc/en/tutorial/melody) This link helped me to let my buzzer play a sound. It is a standard code I found at the arduino website and this is the sound you hear when the alarm goes off. 
...
4. [HTTP:](https://github.com/esp8266/Arduino/blob/master/libraries/ESP8266HTTPClient/examples/BasicHttpClient/BasicHttpClient.ino) This link helped me set up the POST and GET request I needed to connect to the server. Without this library and the example code I wouldn't be able to let my project rely on user input.

5. [JSON:](https://github.com/bblanchon/ArduinoJson/blob/master/examples/JsonParserExample/JsonParserExample.ino) This link helped me decode the door.json value in my NodeMCU code to a string. Thanks to this library the user can turn off the alarm in the browser and my NodeMCU can react to this via the GET request.
